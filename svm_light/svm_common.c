     ...
     /* classifies one example */
{
  ...
  if(((model->kernel_parm.kernel_type == LINEAR) || (model->kernel_parm.kernel_type == FAST_LINEAR)) && (model->lin_weights))
     ...
     /*            not be larger than the weight vector!               */
{
  ...
  for(f=ex->fvec;f;f=f->next)  {
    ...
    // printf("factor sprod_ns: %lf %lf\n",f->factor,sprod_ns(model->lin_weights,f));
  }
  return(sum-model->b);
}
...
double sprod_ns(double *vec_n, SVECTOR *vec_s)
{
  ...
  //  printf("IN sprod_ns ");
  while (ai->wnum) {
    //    printf("%d %lf %lf %lf\n",ai->wnum,vec_n[ai->wnum],ai->weight,sum);
     ...
     /* compute weight vector in linear case and add to model */
{
  ...
  for(i=1;i<=model->totwords;i++) {
    printf("LINEAR_WEIGHTS: %ld %lf\n",i,model->lin_weights[i]); 
  }
...
MODEL *read_model(char *modelfile)
{
  ...
  if(model->kernel_parm.kernel_type == FAST_LINEAR)  {
    
    printf("Will read precalculated linear weights\n"); //fast_linear);

    model->lin_weights=(double *)my_malloc(sizeof(double)*(model->totwords+1));
    for(i=1;i<=model->totwords;i++) {
      fgets(line,(int)ll,modelfl);
      sscanf(line,"LINEAR_WEIGHTS: %d %lf\n", &pos, &model->lin_weights[i]);
